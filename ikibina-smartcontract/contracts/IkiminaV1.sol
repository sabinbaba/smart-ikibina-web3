    ...
    PausableUpgradeable 
{
    ...
    // ── Enums ──────────────────────────────────────────────
    enum LoanStatus { None, Pending, Active, Repaid, Defaulted }
    enum MemberRole { Regular, President, Accountant, ChiefOfMember }
    ...
    // ── Events ─────────────────────────────────────────────
    event MemberRegistered(address indexed wallet, string name, MemberRole role, uint256 timestamp);
    ...
    event LoanRequested(uint256 indexed requestId, address indexed borrower, uint256 amount, bool autoApproved, uint256 timestamp);
    event LoanAutoApproved(address indexed borrower, uint256 amount, uint256 totalOwed, uint256 deadline, uint256 timestamp);
    ...
    event RoleUpdated(address indexed member, MemberRole oldRole, MemberRole newRole, uint256 timestamp);
    event ContractUpgraded(address indexed implementation, string newVersion, uint256 timestamp);
    ...
    modifier onlyMember() {
        require(members[msg.sender].isRegistered && members[msg.sender].isActive, "Ikimina: not an active member");
        _;
    }
    
    modifier onlyLeader() {
        MemberRole role = members[msg.sender].role;
        require(role == MemberRole.President || role == MemberRole.Accountant || role == MemberRole.ChiefOfMember, "Ikimina: not authorized");
        _;
    }
    ...
    function updateMemberRole(address _wallet, MemberRole _newRole) external onlyAdmin {
        require(members[_wallet].isRegistered, "Ikimina: member not registered");
    ...
    function removeMember(address _wallet) external onlyAdmin {
        require(members[_wallet].isRegistered && members[_wallet].isActive, "Ikimina: not an active member");
        require(loans[_wallet].status != LoanStatus.Active && loans[_wallet].status != LoanStatus.Pending, "Ikimina: member has active/pending loan");
    ...
    function contribute() external payable onlyMember nonReentrant whenNotPaused {
        require(msg.value >= MIN_CONTRIBUTION, "Ikimina: below minimum contribution");
        members[msg.sender].savings += msg.value;
        totalPool += msg.value;
        emit ContributionMade(msg.sender, msg.value, members[msg.sender].savings, totalPool, block.timestamp);
    }

    function withdrawSavings(uint256 _amount) external onlyMember nonReentrant whenNotPaused {
        require(_amount > 0, "Ikimina: amount must be > 0");
        require(loans[msg.sender].status != LoanStatus.Active, "Ikimina: repay active loan first");
        require(members[msg.sender].savings >= _amount, "Ikimina: insufficient savings");
        require(totalPool >= _amount, "Ikimina: insufficient pool liquidity");

        members[msg.sender].savings -= _amount;
        totalPool -= _amount;
        emit WithdrawalMade(msg.sender, _amount, members[msg.sender].savings, block.timestamp);
    ...
    function requestLoan(uint256 _amount) external onlyMember whenNotPaused returns (uint256 requestId) {
        require(_amount > 0, "Ikimina: amount must be > 0");
        require(loans[msg.sender].status != LoanStatus.Active && loans[msg.sender].status != LoanStatus.Pending, "Ikimina: existing loan");
        require(_amount <= members[msg.sender].savings * LOAN_MULTIPLIER, "Ikimina: exceeds 3x savings limit");
        require(_amount <= totalPool, "Ikimina: insufficient pool");
        require(_amount <= (totalPool * MAX_LOAN_PERCENTAGE) / 100, "Ikimina: exceeds pool safety limit");
        require(totalPool - _amount >= (totalPool * REQUIRED_RESERVE_PERCENTAGE) / 100, "Ikimina: violates reserve requirement");
        ...
        if (autoApproved) {
            ...
            emit LoanAutoApproved(msg.sender, _amount, _amount + (_amount * INTEREST_BPS) / BPS_DENOMINATOR, block.timestamp + LOAN_DURATION, block.timestamp);
            return 0;
        } else {
            ...
            emit LoanRequested(requestId, msg.sender, _amount, false, block.timestamp);
            return requestId;
        }
    }
    
    function _checkAutoApprovalConditions(uint256 _amount) internal view returns (bool) {
        Member storage member = members[msg.sender];
        if (totalPool < _amount * 2) return false;
        if (member.savings < _amount) return false;
        uint256 maxAutoAmount = (totalPool * AUTO_APPROVAL_MAX_PERCENTAGE) / 100;
        if (_amount > maxAutoAmount) return false;
        if (block.timestamp < member.joinedAt + NEW_MEMBER_TENURE_DAYS * 1 days) return false;
    ...
    function _executeLoanApproval(address _borrower, uint256 _amount) internal {
        ...
        totalPool -= _amount;
        emit LoanDisbursed(_borrower, _amount, totalOwed, deadline, block.timestamp);

        (bool sent, ) = payable(_borrower).call{value: _amount}("");
        require(sent, "Ikimina: ETH transfer failed");
    }

    function approveLoan(uint256 _requestId) external onlyLeader nonReentrant whenNotPaused {
        LoanRequest storage req = loanRequests[_requestId];
        require(!req.approved && !req.rejected, "Ikimina: already processed");
        require(loans[req.borrower].status == LoanStatus.Pending, "Ikimina: not pending");
        require(req.amount <= totalPool, "Ikimina: insufficient pool");

        req.approved = true;
        _executeLoanApproval(req.borrower, req.amount);
        manuallyApprovedCount++;
        emit LoanApproved(_requestId, req.borrower, req.amount, block.timestamp);
    }
    ...
    function repayLoan() external payable onlyMember nonReentrant whenNotPaused {
        ...
        uint256 interestPortion = (payment * (loan.totalOwed - loan.principal)) / loan.totalOwed;
        ...
        emit LoanRepaid(msg.sender, payment, loan.totalOwed - loan.amountRepaid, loanClosed, block.timestamp);
    ...
    function flagDefault(address _borrower) external onlyLeader {
        ...
        emit LoanDefaulted(_borrower, amountOwed + penalty, loan.deadline, block.timestamp);
    }
    ...
    function distributeDividends() external nonReentrant {
        ...
        uint256 dividendPool = (term.interestCollected * DIVIDEND_BPS) / BPS_DENOMINATOR;
        ...
        for (uint256 i = 0; i < memberList.length; i++) {
            ...
            uint256 share = totalSavingsSnapshot > 0
                ? (dividendPool * members[wallet].savings) / totalSavingsSnapshot
                : dividendPool / memberCount;

            members[wallet].pendingDividends += share;
        }

        emit DividendDistributed(currentTermId, dividendPool, dividendPool / memberCount, block.timestamp);
        currentTermId++;
        _startNewTerm();
    }
    ...
    function emergencyWithdraw(address _to, uint256 _amount) external onlyAdmin whenPaused {
        require(_amount <= address(this).balance, "Ikimina: insufficient balance");
    ...
    function getMember(address _wallet) external view returns (
        string memory name, 
        string memory email, 
        string memory phone,
        bool isRegistered, 
        ...
        return (m.name, m.email, m.phone, m.isRegistered, m.isActive,
                m.savings, m.pendingDividends, m.joinedAt, m.role);
    }

    function getLoan(address _wallet) external view returns (
        uint256 principal, 
        uint256 totalOwed, 
        uint256 amountRepaid,
        ...
        uint256 remaining = l.totalOwed > l.amountRepaid ? l.totalOwed - l.amountRepaid : 0;
        return (l.principal, l.totalOwed, l.amountRepaid, remaining, l.deadline, l.status);
    }

    function getCurrentTerm() external view returns (
        uint256 termId, 
        uint256 startTime, 
        uint256 endTime,
        uint256 interestCollected, 
        ...
        return (currentTermId, t.startTime, t.endTime, t.interestCollected, t.distributed);
    }

    function maxLoanAmount(address _wallet) external view returns (uint256) {
        if (!members[_wallet].isRegistered || !members[_wallet].isActive) return 0;
    ...
    function qualifiesForAutoApproval(address _member, uint256 _amount) external view returns (bool) {
        if (!members[_member].isRegistered || !members[_member].isActive) return false;
        if (totalPool < _amount * 2) return false;
        if (members[_member].savings < _amount) return false;
        uint256 maxAutoAmount = (totalPool * AUTO_APPROVAL_MAX_PERCENTAGE) / 100;
        if (_amount > maxAutoAmount) return false;
        if (block.timestamp < members[_member].joinedAt + NEW_MEMBER_TENURE_DAYS * 1 days) return false;
